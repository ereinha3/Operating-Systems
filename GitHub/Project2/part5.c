...
int CLK_TCK = 100;
char* types[3] = {"Command", "I/O", "Compute"};
int lengths[3] = {1, 3, 5};
...
void handle_top(int num, pid_t pid, long time, int type){
	...
	printf("Of type %s allocating %d seconds of CPU time.\n", types[type], lengths[type]);
...
int main(int argc, char* argv[])
{
	...
	if (argc != 2){
		fprintf(stderr, "Usage: ./part5 {filename}\n");
		return 0;
	}
	filename = argv[1];
	fp = fopen(filename, "r");
	...
	for (int i = 0; i<line_number; i++){
		...
		if (!strcmp(command_list[0], "./cpubound")){
			command_array[i] = 2;
		}
		else if (!strcmp(command_list[0], "./iobound")){
			command_array[i] = 1;
		}
		else {
			command_array[i] = 0;
	...
	printf("-------------------------------------------\n");
	printf("Allocated time for different commands:\n");
	for (int i = 0; i<3; i++){
		printf("%s recieves %d seconds per interval\n", types[i], lengths[i]);
	}
	printf("-------------------------------------------\n");
	...
	while (1){
		...
		if (status[curr_process]==0){
			int type = command_array[curr_process];
			int allocation = lengths[type];
			// Set an alarm for the amount of the quantum for the Round Robin Scheduling Algorithm
			alarm(allocation);
			...
			handle_top(curr_process, pid_array[curr_process], time, type);
